...
fs.readdir("./src/servers/commands/", (err, files) => {
  ...
  console.log(`[flowlist.xyz/servers]: ${files.length} command loaded.`); 
...
serverClient.on('ready',async () => {
    console.log("[flowlist.xyz/servers]: Bot successfully connected as "+serverClient.user.tag+".");
    let serversdata = require("../../src/database/models/servers/server.js");
    const servers = await serversdata.find();
    serverClient.user.setPresence({ activity: { type: 'PLAYING', name: 'fs! | '+ servers.length+ ' servers' }, status: "dnd" });
});
